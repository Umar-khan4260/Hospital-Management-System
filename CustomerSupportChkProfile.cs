...
{
    public partial class CustomerSupportChkProfile : Form
    {
        OracleConnection con;
        private string CSemail;
        public CustomerSupportChkProfile(string CSEmail)
        {
            InitializeComponent();
            CSemail = CSEmail;
        }
        ...
        private void CustomerSupportChkProfile_Load(object sender, EventArgs e)
        {
        ...
        private void viewprofile_Click(object sender, EventArgs e)
        {
            try
            {
                con.Open();
                // wrtiting query
                string InsertDate = "SELECT *  FROM Customer_support_Staff WHERE Email=:email";
                ...
                {
                    cmd.Parameters.Add("email", OracleDbType.Varchar2).Value = CSemail;
                    ...
                    using (OracleDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        {
                            MessageBox.Show("Customer Support Staff not found.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
            ...
            else
            {
                try
                {
                    ...
                    using (OracleCommand cmd = new OracleCommand(query, con))
                    {
                        ...
                        cmd.Parameters.Add("OldEmail", OracleDbType.Varchar2).Value = CSemail;

                        int rowsUpdated = cmd.ExecuteNonQuery();
                        if (rowsUpdated > 0)
                        {
                            // Update successful
                            MessageBox.Show("Customer_support_Staff profile updated successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        ...
                        {
                            MessageBox.Show("Failed to update Customer_support_Staff profile.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                }
                catch (Exception ex)
