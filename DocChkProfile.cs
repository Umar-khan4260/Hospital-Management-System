...
{
    public partial class DocChkProfile : Form
    {
        OracleConnection con;
        private string Docemail;
        public DocChkProfile(string docemail)
        {
            InitializeComponent();
            Docemail = docemail;
        }
        ...
        private void DocChkProfile_Load(object sender, EventArgs e)
        {
        ...
        private void viewprofile_Click(object sender, EventArgs e)
        {
            try
            {
                con.Open();
                // wrtiting query
                string InsertDate = "SELECT * FROM Doctor WHERE Email=:email";
                ...
                {
                    cmd.Parameters.Add("email", OracleDbType.Varchar2).Value = Docemail;
            ...
            else
            {
                ...
                try
                {
                    ...
                    using (OracleCommand cmd = new OracleCommand(query, con))
                    {
                        ...
                        cmd.Parameters.Add("OldEmail", OracleDbType.Varchar2).Value = Docemail;

                        int rowsUpdated = cmd.ExecuteNonQuery();
                        if (rowsUpdated > 0)
                        {
                            // Update successful
                            MessageBox.Show("Doctor profile updated successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
