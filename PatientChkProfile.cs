...
{
    public partial class PatientChkProfile : Form
    {
        OracleConnection con;
        private string Patientemail;
        public PatientChkProfile(string patientemail)
        {
            InitializeComponent();
        ...
        private void PatientChkProfile_Load(object sender, EventArgs e)
        {
        ...
        private void viewtab_Click(object sender, EventArgs e)
        {
            try
            {
                con.Open();
                // wrtiting query
                string InsertDate = "SELECT * FROM Patient WHERE Email=:email";
                ...
                {
                    cmd.Parameters.Add("email", OracleDbType.Varchar2).Value = Patientemail;
            ...
            else
            {
                try
                {
                    ...
                    using (OracleCommand cmd = new OracleCommand(query, con))
                    {
                        ...
                        cmd.Parameters.Add("OldEmail", OracleDbType.Varchar2).Value = Patientemail;

                        int rowsUpdated = cmd.ExecuteNonQuery();
                        if (rowsUpdated > 0)
                        {
                            // Update successful
                            MessageBox.Show("Patient profile updated successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
